...
async function loadRoute(db: PrismaClient) {
  ...
  return import("@/app/api/admin/clients/[id]/services/route");
}
...
describe("admin/clients/[id]/services POST", () => {
  it("unauth: throws", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      ...
      await expect(
        POST(makeReq({ method: "POST", body: { serviceType: "accounting" } }), makeParams({ id: "any" }))
      ).rejects.toThrow();
    });
  });
  ...
  it("wrong role (client): throws FORBIDDEN", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      ...
      const { POST } = await loadRoute(tx);
      await expect(
        POST(makeReq({ method: "POST", body: { serviceType: "accounting" } }), makeParams({ id: client.id }))
      ).rejects.toThrow("FORBIDDEN");
    });
  });

  it("bad input (missing serviceType) → 422", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
  ...
  it("happy path: adds a service → 200, ClientService created", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      const client = await createClient(tx);
      const staff = await createStaff(tx);
      ...
      const { POST } = await loadRoute(tx);
      const res = await POST(
        makeReq({ method: "POST", body: { serviceType: "accounting", notes: "First year" } }),
      ...
      const svc = await tx.clientService.findUnique({ where: { id: json.id } });
      expect(svc?.clientId).toBe(client.id);
      expect(svc?.serviceType).toBe("accounting");
    });
  });
});
