...
async function loadRoute(db: PrismaClient) {
  ...
  return import("@/app/api/admin/documents/[id]/route");
}
...
async function createTestDocument(tx: PrismaClient, prospectId: string) {
  return tx.document.create({
    data: {
      prospectId,
      type: "other",
      storageKey: `prospects/${prospectId}/test-doc.pdf`,
...
describe("admin/documents/[id] PATCH", () => {
  it("unauth: throws", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      ...
      await expect(
        PATCH(makeReq({ method: "PATCH", body: { status: "approved" } }), makeParams({ id: "any" }))
      ).rejects.toThrow();
    });
  });
  ...
  it("happy path: updates document status → 200", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      const client = await createClient(tx);
      const prospect = await tx.prospect.findUnique({ where: { id: client.prospectId } });
      const doc = await createTestDocument(tx, prospect!.id);
      const staff = await createStaff(tx);
      sessionState.user = { id: staff.id, email: staff.email, fullName: staff.fullName, role: "staff" };
      const { PATCH } = await loadRoute(tx);
      const res = await PATCH(
        makeReq({ method: "PATCH", body: { status: "approved" } }),
        makeParams({ id: doc.id })
      );
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.ok).toBe(true);
      const updated = await tx.document.findUnique({ where: { id: doc.id } });
      expect(updated?.status).toBe("approved");
    });
  });
});

describe("admin/documents/[id] DELETE", () => {
  it("unauth: throws", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      sessionState.user = null;
      ...
      await expect(
        DELETE(makeReq({ method: "DELETE" }), makeParams({ id: "any" }))
      ).rejects.toThrow();
    });
  });

  it("happy path: deletes doc → 200", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      const client = await createClient(tx);
      const prospect = await tx.prospect.findUnique({ where: { id: client.prospectId } });
      ...
      expect(res.status).toBe(200);
      const deleted = await tx.document.findUnique({ where: { id: doc.id } });
      expect(deleted).toBeNull();
    });
  });

  it("deletion of doc that fulfilled a request reopens the request", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      const client = await createClient(tx);
      const prospect = await tx.prospect.findUnique({ where: { id: client.prospectId } });
      ...
      // Create an open document request
      ...
      // Create a doc that fulfills the request
      ...
      // Doc deleted
      const deletedDoc = await tx.document.findUnique({ where: { id: doc.id } });
      expect(deletedDoc).toBeNull();

      // Request reopened
      const reopened = await tx.documentRequest.findUnique({ where: { id: docReq.id } });
      expect(reopened?.state).toBe("open");
      expect(reopened?.fulfilledDocumentId).toBeNull();
    });
  });
});
