...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const now = new Date();
  // Tools this deployment offers; /tools itself drops out when there are none.
  const tools = await getEnabledTools().catch(() => []);
  const statics = ["", "/services", ...(tools.length > 0 ? ["/tools"] : []), "/insights", "/about", "/contact", "/book", "/faq", "/privacy", "/cookies", "/terms"];
